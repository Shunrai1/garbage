  ...
  <div class="container">
    <div class="step">
      ...
      <div class="content" v-if="scene == 0">
        ...
        <br />
        <br />
        <el-text class="mx-1">选择部署的其他垃圾运输车辆</el-text>
        <br />
      ...
      <div class="content" v-else-if="scene == 2">
        <el-text class="mx-1">输入要到达的中转站点</el-text>
        <br />
        ...
        >
          ...
          <el-table-column prop="deal" label="日处理量/吨"></el-table-column>
        </el-table>
      </div>
...
const stopqueryVectorLayerByPnt = async (sceneArg: any) => {
  ...
  if (sceneArg == 1) {
    ...
    selectStation.value.forEach((element: Feature) => {
      element.setStyle(gStationStyle)
    ...
    await reqStationByIds(stationIds.value.join(','))
  }
}
const drawControlback = (event: any, sceneArg: any) => {
  ...
  map.value?.getLayers().forEach(function (layer) {
    if (
      ...
      layer instanceof VectorLayer &&
      layer.getProperties().title == '垃圾中转站' &&
      sceneArg == 2
    ) {
      const source = layer.getSource()
      source.forEachFeature((pointFeature: any) => {
...
const reqStationByIds = async (ids: string) => {
  const res = await reqStationByIdsAPI(ids)
  if (res.code == 200) {
...
const handler = (gid: any) => {
  ...
  depotRes.value.forEach((element: any) => {
    if (element.gid == gid) {
      rowtrucks.value = element.trucks.filter((v) => v.type == 1)
    }
  })
}
//计算od代价矩阵
const compute = async () => {
  ...
  if (
    ...
    const data = {
      ...
      depotids: c1gid.value,
    }
    reqComputeLMAPI(data)
    mytimer.value = setInterval(reqCheck, 1000)
  }
}

const reqCheck = async () => {
  const res = await reqCheckAPI()
  if (res.code == 200) {
    if (res.data == '完成') {
...
const routeplan = async () => {
  ...
  const res = await routePlanAPI(c2id.value.join(','), data, '1')
  if (res.code == 200) {
